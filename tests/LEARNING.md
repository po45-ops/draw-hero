...
`js/handwriting.js` compares individual ink segments against competing character templates, rather than assigning correctness from a whole-word resemblance score. It compensates for display aspect ratio and pen thickness, replays erasers, and requires every segment to match. A low-confidence or inseparable answer returns “uncertain”, with no immediate wrong-answer record or reward. Normal battle time resumes after the bounded reading grace described below.

Similar shapes are accepted with a small competing-template tolerance; stroke count and drawing order are not checked. This is **not a trained handwriting OCR model** and cannot promise universal accuracy. Connected letters, overlapping Thai marks, very cursive handwriting, punctuation and multi-line sentences may need rewriting with clearer spacing. Spaces are ignored. English words and phrases accept either case, including mixed case; explicitly uppercase/lowercase letter lessons keep their lesson alphabet. Isolated glyphs whose case differs only by size (such as C/c) cannot reliably reveal case after scale normalization. No handwritten data is uploaded by the recognizer.

English ink components are separated in two dimensions, so disjoint characters with overlapping horizontal extents (such as A and a long T crossbar) need not have an empty column between them. Detached dots/crossbars are associated with nearby stems. This does not promise arbitrary joined cursive recognition. Additional pen templates cover rounded A, slanted T and single-storey a/g. Letter alternatives share a case-insensitive identity for word checking but still compete against genuinely different letters. Feedback identifies an uncertain/mismatched position when segmentation succeeds; otherwise it explicitly reports a segmentation issue without pretending to know which letter is wrong. No ink is cleared on these retries.
...
Version 3.1.4 adds reusable pen-path variants for Thai ร and จ and a small penalty for significantly different closed-loop positions when both shapes have measurable loops. It does not enforce an exact loop count, stroke count or stroke order. Confusion pairs ร/ฐ and จ/ค must favor the expected shape rather than allowing both through the usual small margin tolerance. This is a targeted improvement, not full coverage of real Thai handwriting styles.

Uncertain feedback identifies a glyph position when segmentation succeeds, including Thai words, without clearing the ink. Expected digits are not displayed in arithmetic feedback. Recognition computation is excluded from the next animation delta. An uncertain, nonempty submission can grant a 3-second timer/approach pause, at most twice (6 seconds) per question. Repeated casts during an active grace period do not extend it. Pausing the game preserves the remaining grace period; a correct answer ends it. An enemy strike already initiated before submission is not undone. The same question being reset does not replenish the allowance.
...
`node tests/english-handwriting.test.cjs` checks CAT/cat/Cat/cAt/caT at three angles, mixed-case phrases, an independently drawn CAT with overlapping letter columns, wrong spellings/extra/missing letters and explicit case lessons. Optional `DH_CAT_SCREENSHOT=/absolute/path/to/the/reported/screenshot.png` verifies the user's CAT ink extracted in memory from the reported screenshot; that personal image is not committed or published. The exact reported CAT screenshot passes in this test. The changes update only recognition, related feedback/rule text in existing elements and script cache versions, not CSS/layout/assets.

`node tests/thai-handwriting.test.cjs` tests ร/ฐ and จ/ค, words, two fonts and three angles (42 fixtures), plus 16 incorrect-spelling controls. Optional `DH_THAI_SCREENSHOT=/absolute/path/to/the/reported/screenshot.png` extracts กระจก in memory at three ink thresholds and checks that it passes กระจก but not กฐะจก, กระคก, กระจกก or กะจก. This screenshot informed the fix and is a regression test, not independent evidence of population accuracy. No new multi-person handwriting dataset was collected. The learning integration tests cover bounded retry allowance, preserved ink, Thai per-glyph feedback, no arithmetic answer leak, and timer/enemy resume after the grace period; browser tests cover allowance reset only on a new question.
