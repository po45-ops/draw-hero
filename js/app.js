(function () {
  ...
  function startPractice(event){
    ...
    startBattle({id:`practice_${Date.now()}`,name:"ห้องฝึกเวท",world:0,contentType:type,questions:ids,enemyTypes:["goblin","ghost"],timeLimit:Number($("practice-time").value),practice:true,enemySpeed:0,guideMode:$("practice-guide").value,recognitionThreshold:DH.Levels.difficulties[game.difficulty].threshold,rewardXP:0,rewardCoins:0});
  }
  ...
  function nextStage(){if(!currentStage){showScreen("map");return;}const visible=DH.Levels.levels.filter(stage=>!stage.hiddenFromMap),index=visible.findIndex(stage=>stage.id===currentStage.id),next=visible[index+1];if(next)startBattle(next);else{mapView="worlds";showScreen("map");}}
  ...
  function returnToStages(){battle.stop();ar.stop();$("screen-battle").classList.remove("ar-mode");$("pause-modal").hidden=true;if(game.mode==="story"&&currentStage){selectedWorld=currentStage.world;mapView="stages";showScreen("map");}else showScreen("main");}
