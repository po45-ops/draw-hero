(function () {
  ...
  class DrawingBoard {
    constructor(canvas, options) {
      ...
      this.strokes=[];this.redoStack=[];this.activeStroke=null;this.activePointerId=null;this.tool="pen";this.width=13;
    ...
    pointerDown(event){if(this.activeStroke||(event.pointerType==="mouse"&&event.button!==0))return;event.preventDefault();this.canvas.setPointerCapture(event.pointerId);this.activePointerId=event.pointerId;this.activeStroke={tool:this.tool,width:this.width,points:[this.point(event)]};this.strokes.push(this.activeStroke);this.redoStack=[];this.drawDot(this.activeStroke.points[0],this.activeStroke);if(this.options.onStart)this.options.onStart();}
    pointerMove(event){if(!this.activeStroke||event.pointerId!==this.activePointerId)return;event.preventDefault();const samples=event.getCoalescedEvents?event.getCoalescedEvents():[];for(const sample of samples.length?samples:[event]){const next=this.point(sample),points=this.activeStroke.points,previous=points[points.length-1];points.push(next);this.drawSegment(previous,next,this.activeStroke);}}
    pointerUp(event){if(!this.activeStroke||event.pointerId!==this.activePointerId)return;event.preventDefault();this.activeStroke=null;this.activePointerId=null;if(this.options.onChange)this.options.onChange(this.strokes);}
    ...
    clear(notify=true){this.strokes=[];this.redoStack=[];this.activeStroke=null;this.activePointerId=null;this.redraw();if(notify&&this.options.onChange)this.options.onChange(this.strokes);}
