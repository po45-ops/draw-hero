(function () {
  ...
  class DrawingBoard {
    ...
    drawGuide(){if(!this.guide||this.guideMode==="none"||this.guideOpacity<=0)return;const ctx=this.ctx;ctx.save();ctx.globalAlpha=(this.guideMode==="full"?.22:.3)*this.guideOpacity;ctx.fillStyle="#46617a";ctx.strokeStyle="#46617a";ctx.textAlign="center";ctx.textBaseline="middle";ctx.font=`700 ${Math.min(300,this.canvas.height*.7)}px Tahoma, Arial, sans-serif`;if(this.guideMode==="dotted"){ctx.setLineDash([7,10]);ctx.lineWidth=3;ctx.strokeText(this.guide,this.canvas.width/2,this.canvas.height/2+10);}else ctx.fillText(this.guide,this.canvas.width/2,this.canvas.height/2+10);ctx.restore();}
